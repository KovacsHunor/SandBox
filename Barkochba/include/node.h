...
class Node {
   ...
   public:
	Node(std::string name, Node *no = nullptr, Node *yes = nullptr)
		: name(name), no(no), yes(yes) {}
	std::string getName() { return name; }
	bool isLeaf() { return no == nullptr && yes == nullptr; }
	void setName(std::string str) { name = str; }
	void setYes(Node *n) { yes = n; }
	void setNo(Node *n) { no = n; }
	...
	void finalTask(bool yes) {
		...
		if (str == "nem") {
			...
			std::cout << "milyen eddig nem elhangzott kérdés igaz rá, ami nem igaz erre: \""
					  << l->getName() << "\"?" << std::endl;
